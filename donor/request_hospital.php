...
if(isset($_SESSION)){
  $con = mysqli_connect('localhost', 'root', '', 'multi_login');
    ...
    $query="SELECT donor_id, donor_first_name, donor_last_name FROM donor WHERE user_id=$user_id";    
    $data=mysqli_query($con, $query);
    $row=$data->fetch_array();
    extract($row);
    if(isset($_POST['link_btn'])){
        extract($_POST);
        $query = "INSERT INTO donor_hospital(donor_id, hospital_id) VALUES($donor_id, $hospital_id)";
